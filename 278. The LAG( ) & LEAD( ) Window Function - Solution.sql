...
# - column showing the previous salary from the given ordered list
...
# -  column displaying the difference between the next salary of a certain employee and their current salary
...
FROM
employees.salaries
WHERE emp_no >= 10500 AND emp_no <= 10600
AND salary > 80000
...
FROM
employees.salaries
WHERE emp_no >= 10001 AND emp_no <= 10100
WINDOW w AS (PARTITION BY emp_no ORDER BY salary ASC);
